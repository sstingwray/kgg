...
const TEXT_START$1 = 123; // {
const TEXT_END$1 = 125; // }
const ESCAPE$1 =  92; // \ character
...
const DOT$2        = 46; // .
const EQUALS$1     = 61; // =
const ATTR_OPEN$1  = 91; // [
const ATTR_CLOSE$1 = 93; // ]

const reAttributeName$1 = /^\!?[\w\-:\$@]+\.?$/;
...
function consumeElement$1(stream) {
	// consume element name, if provided
	const start = stream.pos;
	const node = new Node(eatName$1(stream));
	let next;
...
const GROUP_START$1 = 40; // (
const GROUP_END$1   = 41; // )
const OP_SIBLING$1  = 43; // +
const OP_CHILD$1    = 62; // >
const OP_CLIMB$1    = 94; // ^
...
function parse$1(str) {
	...
	while (!stream.eof()) {
		...
		if (ch === GROUP_START$1) { // start of group
			...
			// a group can have a repeater
			if (node.repeat = consumeRepeat$1(stream)) {
				ctx.appendChild(node);
			} else {
				// move all children of group into parent node
				while (node.firstChild) {
		...
		const node = consumeElement$1(stream);
		ctx.appendChild(node);
 ...
 * @return {Node}
 */
function index$1(abbr) {
	const tree = parse$1(abbr);
	tree.walk(unroll$1);
	return tree;
}
...
var index$2 = function(tree, registry) {
...
function resolveNode(node, registry) {
    ...
    const resolve = node => {
        ...
        const tree = index$1(snippet.value);
 ...
 */
var index$3 = function(tree, content, appliedAddons) {
 ...
 * @return {String}
 */
function index$4(tree, profile, syntax, options) {
...
function expand(abbr, config) {
	...
	return index$4(abbr, config.profile, config.syntax, config);
}
...
function parse$3(abbr, config) {
	return index(abbr)
		.use(index$2, config.snippets)
		.use(replaceVariables, config.variables)
		.use(index$3, config.text, config.options);
}
...
function isIdentPrefix(code) {
	return code === AT$1 || code === DOLLAR$1$1 || code === EXCL$2;
}
...
function index$6(tree, registry, options) {
 ...
 * @return {Node}
 */
function parse$4(abbr, config) {
	if (typeof abbr === 'string') {
		abbr = index$5(abbr);
	}

	return abbr.use(index$6, config.snippets, config.options);
}
